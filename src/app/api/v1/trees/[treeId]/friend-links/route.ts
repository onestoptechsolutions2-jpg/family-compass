...
/**
 * Cross-tree friend links touching this tree — the "union of trees" edges.
 * Person + tree ids only, no names and no free-text origin, so this is safe
 * to sync without leaking PII. Each row appears once for the tree on either
 * side of the link.
 */
export async function GET(req: NextRequest, { params }: { params: Promise<{ treeId: string }> }) {
  ...
  const rows = await db.friendLink.findMany({
    where: { OR: [{ aTreeId: treeId }, { bTreeId: treeId }] },
    orderBy: { id: "asc" },
    take: limit + 1,
