...
const schema = z.object({
  ...
  memorialPassKes: z.coerce.number().int().min(1).max(10_000_000).default(1500),
  memorialPassDays: z.coerce.number().int().min(1).max(3650).default(120),
...
export async function updatePaymentSettings(formData: FormData) {
  ...
  const data = {
    ...
    memorialPassKes: d.memorialPassKes,
    memorialPassDays: d.memorialPassDays,
